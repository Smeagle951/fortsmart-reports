...
interface HeaderSectionProps {
  ...
  onExportPdf?: () => void;
  onCompartilhar?: () => void;
  /** Barra só com status + ações (evita repetir logo/dados já no cabeçalho institucional). */
...
export default function HeaderSection({
  ...
  if (variant === 'toolbar') {
    return (
      <header
        className={`saas-header sticky top-0 z-40 border-b border-slate-200 bg-white/98 backdrop-blur-md shadow-sm print:hidden ${className}`.trim()}
      >
        <div className="h-0.5 bg-gradient-to-r from-emerald-600 to-emerald-500" aria-hidden />
        <div className="mx-auto max-w-7xl px-4 py-2.5 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-end gap-3">
            <span className="mr-auto text-xs text-slate-500 hidden sm:inline">Ações do relatório</span>
  ...
  return (
    <header className={`saas-header sticky top-0 z-40 border-b border-slate-200 bg-white/98 backdrop-blur-md shadow-sm print:static print:bg-white ${className}`.trim()}>
      <div className="h-0.5 bg-gradient-to-r from-emerald-600 to-emerald-500" aria-hidden />
