...
export function normalizeSituacaoSlug(s: string | undefined): SituacaoSlug {
  if (!s) return 'saudavel';
  const x = foldAccents(s);
  if (x.includes('crit')) return 'critico';
  if (x.includes('aten')) return 'atencao';
  return 'saudavel';
}

export function normalizeRiscoSlug(s: string | undefined): RiscoSlug {
  if (!s) return 'baixo';
  const x = foldAccents(s);
...
export function normalizeTendenciaSlug(s: string | undefined): TendenciaSlug {
  if (!s) return 'estavel';
  const x = foldAccents(s);
...
export function formatSituacaoDisplay(slug: string | undefined): string {
...
export function formatRiscoDisplay(slug: string | undefined): string {
  const r = normalizeRiscoSlug(slug);
  if (r === 'alto') return 'Alto';
  if (r === 'moderado') return 'Moderado';
  return 'Baixo';
}

export function formatTendenciaDisplay(slug: string | undefined): string {
...
function impactoFromEconomia(m: InteligenciaAgronomicaPayload): { perda_estimada_sc: number; roi_estimado: number } {
  ...
  if (ec && typeof ec === 'object') {
    ...
    const r = ec.roi_estimado;
    if (typeof ph === 'number' && !Number.isNaN(ph)) perda = ph;
    if (typeof r === 'number' && !Number.isNaN(r)) roi = r;
  }
...
export function normalizeInteligenciaPayload(raw: Record<string, unknown>): InteligenciaAgronomicaPayload {
  const out: InteligenciaAgronomicaPayload = {
    ...
    tendencia: normalizeTendenciaSlug(raw.tendencia as string | undefined),
  ...
  const resumoTrim = (out.resumo ?? '').trim();
...
function normSeveridade(s: string | undefined): number {
  if (!s) return 0;
